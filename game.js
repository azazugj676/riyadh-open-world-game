...
const COLS = 20;
const ROWS = 20;
const CELLS = canvas.width / COLS; // assumes square canvas
...
const TICK_SPEED = 120; // game speed (ms per tick)
...
function init() {
    ...
    gameInterval = setInterval(tick, TICK_SPEED);
}
...
function draw() {
    ...
    for (let y = 0; y <= ROWS; y++) {
        ctx.beginPath();
        ctx.moveTo(0, y * CELLS);
        ctx.lineTo(canvas.width, y * CELLS);
        ctx.stroke();
    }
}

function drawCell(x, y) {
    ctx.fillRect(x * CELLS + 1, y * CELLS + 1, CELLS - 2, CELLS - 2);
}
